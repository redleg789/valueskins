...
export default function BrandMarketplacePage() {
  ...
  const formatReward = (amount: i64 | undefined, currency: string) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const oppsResult = await fetch(`/matching/persona/${personaId}/opportunities?limit=100`, {
        credentials: 'include',
  ...
  const handleSendOffer = async (opportunityId: number) => {
    // This would open a deal room creation flow or notification to brand
    // For now, show success and notify user to navigate to deal room
            ...
            {opportunities.map(opp => (
              <div key={opp.opportunity_id} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '16px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  <button
                    onClick={() => handleSendOffer(opp.opportunity_id as unknown as number)}
