        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999 }} onClick={() => setAskModalOpp(null)}>
          <div style={{ background: C.surface, borderRadius: '16px', padding: '24px', maxWidth: '480px', width: '95vw', maxHeight: '90vh', overflowY: 'auto', border: `1px solid ${C.border}`, position: 'relative' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
              ...
              <div>
                <a href={askModalOpp.instagramUrl || `https://linkedin.com/in/${askModalOpp.brand.replace(/\s+/g, '').toLowerCase()}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: '16px', fontWeight: 700, color: C.text, textDecoration: 'none', display: 'block' }}>{askModalOpp.brand}</a>
      ...
      {!isMobile && (
        <div style={{ width: '240px', borderRight: `1px solid ${C.border}`, padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: '16px', height: '100vh', overflowY: 'auto', position: 'sticky', top: 0, background: C.bg, flexShrink: 0 }}>
          <Link href="/" style={{ textDecoration: 'none' }}>
            <div style={{ fontSize: '22px', fontWeight: 700, color: C.text, marginBottom: '16px', cursor: 'pointer', fontStyle: 'italic' }}>LinkedIn</div>
          </Link>
          ...
          {activeView === 'profile' && (
            <>
              ...
              <div style={{ borderTop: `1px solid ${C.border}`, display: 'flex' }}>
                {['posts', 'articles', 'network', 'insights', 'deals', 'uploaded'].map((tab) => (
                          ...
                          return (
                            ...
                            >
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <a href={opp.instagramUrl || `https://linkedin.com/in/${opp.brand.replace(/\s+/g, '')}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ fontSize: '15px', fontWeight: 700, color: C.text, textDecoration: 'none', display: 'block' }}>{opp.brand}</a>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                      <div style={{ fontSize: '15px', fontWeight: 700, color: C.text }}>
                                        Deal Room · <a href={opp.instagramUrl || `https://linkedin.com/in/${opp.brand.replace(/\s+/g, '')}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ color: C.primary, textDecoration: 'none' }}>{opp.brand}</a>
                                      </div>
                                          ...
                                          <button
                                            ...
                                            onClick={() => {
                                              ...
                                              const newApp: SharedApplication = {
                                                ...
                                                creatorResponseTimeHrs: 6, creatorInstagramUrl: `https://linkedin.com/in/creator_demo`,
                                                opportunityIndex: actualOppIndex,
                                              };
                                    ...
                                    switch(dealType) {
                                      ...
                                      case 'c2c_paid': {
                                        ...
                                        if (creatorDealLifecycle === 'deliverables') {
                                          const oppDeliverables = opp.deliverables?.length ? opp.deliverables : [{ format: 'LinkedIn Post', count: 1 }];
                                                    ...
                                                    <div key={di} style={{ borderRadius:'8px', marginBottom:'8px', border:`1px solid ${status === 'approved' ? 'rgba(0,212,106,0.25)' : status === 'uploaded' ? 'rgba(0,149,246,0.25)' : C.border}`, overflow:'hidden' }}>
                                                      <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'8px 10px', background: status === 'uploaded' ? 'rgba(0,149,246,0.04)' : status === 'approved' ? 'rgba(0,212,106,0.04)' : 'transparent' }}>
                                                        ...
                                                        <div style={{ flex:1 }}>
                                                          ...
                                                          <div style={{ fontSize:'10px', color: status === 'approved' ? C.success : status === 'uploaded' ? C.primary : C.textMuted }}>
                                                            {status === 'approved' ? 'Approved by brand' : status === 'uploaded' ? 'Submitted — awaiting review' : status === 'linking' ? 'Enter your LinkedIn post link below' : 'Not yet submitted'}
                                                          </div>
                                                        </div>
                                                        {status === 'pending' && (
                                                      ...
                                                      {status === 'linking' && (
                                                        <div style={{ padding:'10px', borderTop:`1px solid ${C.border}`, background:C.bg }}>
                                                          <div style={{ fontSize:'10px', color:C.textMuted, marginBottom:'6px' }}>Paste the link to your published LinkedIn post</div>
                                                          <div style={{ display:'flex', gap:'6px' }}>
                                                            <input
                                                              type="text"
                                                              value={inputVal}
                                                          ...
                                                          {inputVal && !isValidIgUrl(inputVal) && (
                                                            <div style={{ fontSize:'10px', color:'#ef4444', marginTop:'4px' }}>Must be a linkedin.com/posts or linkedin.com/feed/update link</div>
                                                          )}
                                                        </div>
                                                      )}
                                                      {(status === 'uploaded' || status === 'approved') && link && (
                                                        <div style={{ padding:'10px', borderTop:`1px solid ${C.border}`, background:C.bg, display:'flex', gap:'10px', alignItems:'flex-start' }}>
                                                          ...
                                                          <div style={{ flex:1, minWidth:0 }}>
                                                            ...
                                                            <a href={link} target="_blank" rel="noopener noreferrer" style={{ fontSize:'10px', color:C.primary, textDecoration:'none', display:'block', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{postId ? `linkedin.com/posts/${postId}` : link}</a>
                                          ...
                                          <>
                                            ...
                                            {status === 'content_due' && (
                                              <div style={{ background:'rgba(59,130,246,0.06)', border:`1px solid rgba(59,130,246,0.2)`, borderRadius:'8px', padding:'12px', marginBottom:'12px' }}>
                                                ...
                                                <div style={{ fontSize:'11px', color:C.textSecondary, marginBottom:'8px' }}>Submit a link to your LinkedIn post featuring the product</div>
                                                <input
                                                  type="text"
                                      ...
                                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        ...
                                        <button
                                          onClick={() => {
                                            ...
                                            if (pocHandle) {
                                              window.open(`https://linkedin.com/in/${pocHandle}`, '_blank');
                                            }
                                            setPurchaseToast(`Opening ${activeDeal.poc?.name || 'POC'} on LinkedIn`);
                                            setTimeout(() => setPurchaseToast(null), 2000);
                                          }}
                      ...
                      <>
                        ...
                        {campaigns.length === 0 ? (
                          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:'12px', padding:'22px 18px', textAlign:'center' }}>
                            <div style={{ fontSize:'16px', fontWeight:700, color:C.text, marginBottom:'6px' }}>Create a campaign first</div>
                            <div style={{ fontSize:'12px', color:C.textSecondary, lineHeight:1.5, marginBottom:'14px' }}>
                              Creator suggestions unlock after you publish your first campaign brief.
                            </div>
                            <button onClick={() => setMarketplaceTab('campaigns')} style={{ background:C.primary, border:'none', borderRadius:'8px', padding:'10px 14px', color:'#fff', fontWeight:700, fontSize:'12px', cursor:'pointer' }}>
                              Create Campaign
                            </button>
                  ...
                  <div style={{ padding: '0 16px 16px' }}>
                    ...
                    {showCampaignCreator && (
                      <div style={{ position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.75)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999 }}>
                        <div style={{ background:C.surface, borderRadius:'16px', padding:'24px', maxWidth:'480px', width:'95vw', maxHeight:'90vh', overflowY:'auto', border:`1px solid ${C.border}`, position:'relative' }}>
                          ...
                          <div style={{ marginBottom:'12px' }}>
                            <div style={{ fontSize:'11px', color:C.textMuted, fontWeight:600, marginBottom:'4px' }}>About the role or project *</div>
                          ...
                          <div style={{ marginBottom:'12px', background:`${C.primary}08`, border:`1px solid ${C.primary}30`, borderRadius:'8px', padding:'10px 12px' }}>
                            <div style={{ fontSize:'11px', color:C.textMuted, fontWeight:600, marginBottom:'4px' }}>Targeting experts with</div>
                        ...
                        <div style={{ background:C.surface, borderRadius:'16px', padding:'24px', maxWidth:'500px', width:'100%', maxHeight:'90vh', overflowY:'auto', border:`1px solid ${C.border}`, position:'relative' }}>
                          ...
                          <div style={{ display:'flex', gap:'12px', alignItems:'flex-start', marginBottom:'16px' }}>
                            ...
                            <div style={{ flex:1 }}>
                              ...
                              <a href={`https://linkedin.com/in/${selectedProfileCreator.handle.replace('@', '')}`} target="_blank" rel="noopener noreferrer" style={{ fontSize:'12px', color:C.primary, textDecoration:'none' }}>{selectedProfileCreator.handle}</a>
                    ...
                    {campaigns.length === 0 ? (
                      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:'12px', padding:'22px 18px', marginBottom:'16px', textAlign:'center' }}>
                        <div style={{ fontSize:'16px', fontWeight:700, color:C.text, marginBottom:'6px' }}>Create a campaign first</div>
                        <div style={{ fontSize:'12px', color:C.textSecondary, lineHeight:1.5, marginBottom:'14px' }}>
                          Candidate suggestions unlock after you publish your first campaign brief. Once the role and requirements are set, matching professionals will appear here.
                        </div>
                        <button onClick={() => setMarketplaceTab('campaigns')} style={{ background:C.primary, border:'none', borderRadius:'8px', padding:'10px 14px', color:'#fff', fontWeight:700, fontSize:'12px', cursor:'pointer' }}>
                          Go to Campaigns
                        </button>
                    ...
                    <div style={{ background:'rgba(16,185,129,0.06)', border:`1px solid rgba(16,185,129,0.2)`, borderRadius:'10px', padding:'12px 16px', marginBottom:'16px' }}>
                      <div style={{ fontSize:'12px', fontWeight:700, color:C.text, marginBottom:'4px' }}>LinkedIn Expert Hiring System</div>
                      ...
                      return (
                        <div key={i} data-creator-idx={origIdx} style={{ background: C.card, borderRadius: '10px', padding: '12px', marginBottom: '10px', border: `1px solid ${isNegotiating ? C.primary : C.border}`, display: 'flex', alignItems: 'center', gap: '10px' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <a href={`https://linkedin.com/in/${creator.handle.replace('@', '')}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ fontSize: '11px', color: C.primary, textDecoration: 'none' }}>{creator.handle}</a>
                          </div>
                            ...
                            (() => {
                              ...
                              return (
                                <div>
                                  ...
                                  >
                                    {noBrandSkin ? 'No ValueSkin' : hasMatch ? (Object.keys(dealStates).find(key => key.startsWith(`${creator.name}|${creator.valueSkin}|`) && dealStates[key]?.phase && dealStates[key]?.phase !== 'brief') ? 'View Deal' : 'Create Hiring Brief') : 'No Shared ValueSkin'}
                                  </button>
                                </div>
                              );
                            })()
                                        ...
                                        <div style={{ marginTop:'12px' }}>
                                          ...
                                          {contentLink ? (
                                            <div style={{ background:C.bg, borderRadius:'8px', padding:'12px', border:`1px solid ${C.border}`, marginBottom:'10px' }}>
                                              <div style={{ fontSize:'12px', fontWeight:600, color:C.text, marginBottom:'4px' }}>LinkedIn Post</div>
                                      ...
                                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        ...
                                        <button
                                          onClick={() => {
                                            ...
                                            setPurchaseToast(`Opening ${brandDeal.poc?.name || 'POC'} on LinkedIn`);
                                            setTimeout(() => setPurchaseToast(null), 2000);
                                          }}
                                          style={{ background: C.primary, border: 'none', borderRadius: '6px', padding: '5px 12px', color: '#fff', fontSize: '10px', fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}
                                        >
                    ...
                    <div style={{ marginTop:'16px' }}>
                      ...
                      {sharedApplications.filter(a => a.status !== 'invited').length === 0 ? (
                        <div style={{ textAlign:'center', padding:'24px 20px', color:C.textMuted }}>
                          ...
                          <div style={{ fontSize:'11px' }}>Experts will appear here once they apply to your hiring brief.</div>
                        </div>
                        ...
                        const igUrl = app.creatorInstagramUrl || `https://linkedin.com/in/${app.creatorHandle.replace('@', '')}`;
          ...
          {activeView === 'admin' && (
            <>
              ...
              <div style={{ padding: '20px', borderTop: `1px solid ${C.border}` }}>
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <div style={{ fontSize: '14px', fontWeight: 700, color: C.text }}>Platform DMs (LinkedIn)</div>
