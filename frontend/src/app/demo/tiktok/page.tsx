...
function isValidTiktokVideoUrl(url: string): boolean {
  ...
  return /tiktok\.com\/@[^/]+\/video\/\d+/.test(url) || /vm\.tiktok\.com\/[A-Za-z0-9]+/.test(url);
}
...
export default function TikTokDemoPage() {
  ...
  // Auto-select single visible skin if only one is available for discovery
  useEffect(() => {
    const ownedProfessions = (Object.entries(valueSkins) as [ValueSkinSlot, { profession: string; aboutMe: string } | undefined][])
      .filter(([slot, entry]) => !!entry?.profession && !hiddenSkins.has(slot))
    ...
    if (selectedMarketplaceSkin && !ownedProfessions.includes(selectedMarketplaceSkin)) {
      setSelectedMarketplaceSkin(ownedProfessions[0] ?? null);
    }
  }, [valueSkins, hiddenSkins, selectedMarketplaceSkin]);
  ...
  const scriptMode = (activeDeal?.scriptMode as 'non_negotiable' | 'discussion' | 'creator_freedom' | undefined) ?? 'discussion';
  ...
  const hasVisibleValueSkin = (Object.entries(valueSkins) as [ValueSkinSlot, { profession: string; aboutMe: string } | undefined][])
    .some(([slot, entry]) => !!entry?.profession && !hiddenSkins.has(slot));
  ...
  const visibleOwnedSkins = ownedSkins.filter(({ slot }) => !hiddenSkins.has(slot));
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', display: 'flex', color: C.text, fontFamily: '"Proxima Nova", "Sofia Pro", "Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif', overflowX: 'hidden' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999 }} onClick={() => setAskModalOpp(null)}>
          <div style={{ background: C.surface, borderRadius: '16px', padding: '24px', maxWidth: '480px', width: '95vw', maxHeight: '90vh', overflowY: 'auto', border: `1px solid ${C.border}`, position: 'relative' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
              ...
              <div>
                <a href={askModalOpp.instagramUrl || `https://www.tiktok.com/@${askModalOpp.brand.replace(/\s+/g, '').toLowerCase()}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: '16px', fontWeight: 700, color: C.text, textDecoration: 'none', display: 'block' }}>{askModalOpp.brand}</a>
      ...
      {!isMobile && (
        <div style={{ width: '248px', borderRight: `1px solid ${C.border}`, padding: '18px 14px', display: 'flex', flexDirection: 'column', gap: '14px', height: '100vh', overflowY: 'auto', position: 'sticky', top: 0, background: C.surface, flexShrink: 0 }}>
          <Link href="/" style={{ textDecoration: 'none' }}>
            <div style={{ fontSize: '24px', fontWeight: 800, color: C.text, marginBottom: '12px', cursor: 'pointer', letterSpacing: '-0.4px' }}>
              TikTok <span style={{ color: C.primary }}>ValueSkins</span>
            </div>
          </Link>
        ...
        <div style={{ width: '100%', maxWidth: isMobile ? '390px' : '680px', borderLeft: isMobile ? 'none' : `1px solid ${C.border}`, borderRight: isMobile ? 'none' : `1px solid ${C.border}`, background: C.surface, overflowX: 'hidden', minHeight: '100vh' }}>
          ...
          {activeView === 'profile' && (
            <>
              ...
              >
                {/* TikTok desktop layout: large avatar left, info right */}
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: isMobile ? '16px' : '32px', marginBottom: '20px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: 700, color: C.text, letterSpacing: '-0.2px' }}>sakethvelamuri</span>
                      <button
                        onClick={handleFollow}
                        style={{ background: isFollowing ? C.surfaceAlt : C.primary, border: `1px solid ${isFollowing ? C.border : C.primary}`, borderRadius: '8px', color: isFollowing ? C.text : '#fff', padding: '7px 16px', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}
                      >
                        {isFollowing ? 'Following' : 'Follow'}
                      </button>
                      <button style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: '8px', color: C.text, padding: '7px 16px', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}>Message</button>
                    </div>
              ...
              {/* Tabs */}
              <div style={{ borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}`, display: 'flex', background: C.surface }}>
                {['posts', 'reels', 'tagged', 'insights', 'deals'].map((tab) => (
                  <div key={tab} onClick={() => setActiveTab(tab)} style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', height: '46px', borderBottom: activeTab === tab ? `2px solid ${tab === 'insights' ? C.primary : C.text}` : '2px solid transparent', color: activeTab === tab ? (tab === 'insights' ? C.primary : C.text) : C.textMuted, cursor: 'pointer', fontWeight: activeTab === tab ? 700 : 500, fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.4px' }}>
                    {tab === 'posts' ? 'Videos' : tab === 'reels' ? 'LIVE' : tab === 'tagged' ? 'Tagged' : tab === 'insights' ? 'Insights' : 'Deals'}
                  </div>
                ))}
              </div>
              ...
              {activeTab !== 'insights' && activeTab !== 'deals' && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2px', background: C.surface }}>
                  {[...Array(9)].map((_, i) => (
          ...
          {activeView === 'mim' && (
            <>
              ...
              {!hasVisibleValueSkin && marketplaceRole === 'creator' && (
                <div style={{ padding: '48px 16px', textAlign: 'center' }}>
                  ...
                  <h2 style={{ fontSize: '20px', fontWeight: 700, color: C.text, marginBottom: '8px' }}>No Discoverable ValueSkin</h2>
                  <p style={{ fontSize: '14px', color: C.textSecondary, marginBottom: '24px', lineHeight: 1.6, maxWidth: '360px', margin: '0 auto 24px' }}>
                    Your current ValueSkins are hidden from discovery. Unhide a skin or get a new one to access creator-side marketplace matching.
                  </p>
                  <button onClick={() => setActiveView('store')} style={{ background: C.primary, border: 'none', borderRadius: '10px', color: '#fff', padding: '12px 28px', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}>
                    Go to Store
                  </button>
                </div>
              )}
              ...
              {hasVisibleValueSkin && marketplaceRole === 'creator' && (
                <>
                  {/* Marketplace header — TikTok style */}
                  <div style={{ padding: '12px 16px 0', position: 'sticky', top: 0, background: C.bg, zIndex: 10 }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '12px', scrollbarWidth: 'none' }}>
                      {visibleOwnedSkins.map(s => s.profession).map(skin => {
                  ...
                  <div style={{ padding: '0 16px 16px' }}>
                    ...
                    {/* Feature 3: Market rate intelligence panel (Meta data source: TikTok Ads historical rates for creator category) */}
                          ...
                          return (
                            ...
                            >
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <a href={opp.instagramUrl || `https://www.tiktok.com/@${opp.brand.replace(/\s+/g, '')}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ fontSize: '15px', fontWeight: 700, color: C.text, textDecoration: 'none', display: 'block' }}>{opp.brand}</a>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                      <div style={{ fontSize: '15px', fontWeight: 700, color: C.text }}>
                                        Deal Room · <a href={opp.instagramUrl || `https://www.tiktok.com/@${opp.brand.replace(/\s+/g, '')}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ color: C.primary, textDecoration: 'none' }}>{opp.brand}</a>
                                      </div>
                                    ...
                                    <>
                                      <div style={{ background: 'rgba(0,149,246,0.06)', borderRadius: '8px', padding: '12px', marginBottom: '12px', border: `1px solid rgba(0,149,246,0.2)` }}>
                                        ...
                                        {scriptMode === 'non_negotiable' && (
                                          <div style={{ marginTop:'8px', background:'rgba(239,68,68,0.06)', border:'1px solid rgba(239,68,68,0.25)', borderRadius:'6px', padding:'8px' }}>
                                            <div style={{ fontSize:'10px', fontWeight:700, color:'#ef4444', textTransform:'uppercase', marginBottom:'4px' }}>Fixed script detail (non-negotiable)</div>
                                            <div style={{ fontSize:'11px', color:C.textSecondary, whiteSpace:'pre-wrap', lineHeight:1.4 }}>{brandScriptText || 'Brand script not provided'}</div>
                                          </div>
                                        )}
                                      </div>
                                        ...
                                        {scriptMode !== 'non_negotiable' ? (
                                          ...
                                          <button
                                            onClick={() => {
                                              ...
                                              setPurchaseToast('Offer rejected (non-negotiable script)');
                                              setTimeout(() => setPurchaseToast(null), 2500);
                                            }}
                                            style={{ flex: 1, background: 'transparent', border: `1px solid rgba(239,68,68,0.35)`, padding: '9px', borderRadius: '8px', color: 'rgba(239,68,68,0.9)', fontWeight: 600, cursor: 'pointer', fontSize: '12px' }}
                                          >
                                            Reject
                                          </button>
                                        )}
                                  ...
                                  {dealRoomPhase === 'formal_offer' && (() => {
                                    ...
                                    return (
                                      <>
                                        ...
                                        {(scriptMode === 'non_negotiable' || scriptMode === 'discussion' || scriptMode === 'creator_freedom') && (
                                          <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: '10px', padding: '12px', marginBottom: '10px' }}>
                                            <div style={{ fontSize: '10px', fontWeight: 700, color: C.textMuted, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '6px' }}>Scripting</div>
                                            <div style={{ fontSize:'12px', color:C.text, fontWeight:600, marginBottom:'4px' }}>
                                              {scriptMode === 'non_negotiable' ? 'Mode: Non-negotiable script' : scriptMode === 'discussion' ? 'Mode: Discussion in chatroom' : 'Mode: Creator freedom'}
                                            </div>
                                            {scriptMode === 'non_negotiable' && (
                                              <div style={{ fontSize:'11px', color:C.textSecondary, whiteSpace:'pre-wrap', lineHeight:1.4 }}>{brandScriptText || 'Brand-fixed script will apply.'}</div>
                                            )}
                                          </div>
                                        )}
                                          ...
                                          <button
                                            ...
                                            onClick={() => {
                                              ...
                                              const newApp: SharedApplication = {
                                                ...
                                                creatorResponseTimeHrs: 6, creatorInstagramUrl: `https://www.tiktok.com/@creator_demo`,
                                              };
                                    ...
                                    switch(dealType) {
                                      ...
                                      case 'c2c_paid': {
                                        ...
                                        if (creatorDealLifecycle === 'scripting') {
                                          ...
                                          const mutualScriptApproved = creatorScriptApproved && brandScriptApproved;
                                          return (
                                            <>
                                              ...
                                              <div style={{ fontSize:'11px', color:C.textSecondary, marginBottom:'8px' }}>
                                                Mode: <strong style={{ color:C.text }}>
                                                  {scriptMode === 'non_negotiable' ? 'Non-negotiable' : scriptMode === 'discussion' ? 'Discussion' : 'Creator freedom'}
                                                </strong>
                                              </div>
                                              <div style={{ background:C.bg, border:`1px solid ${C.border}`, borderRadius:'8px', padding:'10px', marginBottom:'10px' }}>
                                                ...
                                                <textarea
                                                  ...
                                                  onChange={e => {
                                                    ...
                                                    if (activeDealKey && scriptMode === 'discussion') {
                                                      updateDeal(activeDealKey, { creatorScriptApproved: false, brandScriptApproved: false });
                                                    }
                                                  }}
                                                  placeholder="Write a short script/caption outline for brand approval..."
                                                  rows={6}
                                                  disabled={(scriptStatus === 'submitted' || scriptStatus === 'approved') && scriptMode !== 'discussion'}
                                              ...
                                              {scriptMode !== 'discussion' && scriptStatus !== 'approved' && (
                                                <button
                                                ...
                                                <>
                                                  <button
                                                    ...
                                                    onClick={() => {
                                                      ...
                                                      scriptStatus: 'submitted',
                                                      brandApprovalPhase: 'script_review',
                                                      creatorScriptApproved: true,
                                                      brandScriptApproved: false,
                                                    });
                                                      firebaseSendNotification(opp?.brand || 'Brand', 'application', 'Creator clicked: Yes, I approve this script.');
                                                    }}
                                                    style={{ width:'100%', background: canSubmitScript ? `${C.primary}` : C.border, border:'none', padding:'10px', borderRadius:'8px', color:'#fff', fontWeight:700, cursor: canSubmitScript ? 'pointer' : 'not-allowed', fontSize:'13px', marginBottom:'8px', opacity: canSubmitScript ? 1 : 0.5 }}
                                                  >
                                                    Yes, I approve this script
                                                  </button>
                                                  <div style={{ fontSize:'11px', color:C.textSecondary, marginBottom:'8px' }}>
                                                    Creator approval: <strong style={{ color: creatorScriptApproved ? C.success : C.textMuted }}>{creatorScriptApproved ? 'Approved' : 'Pending'}</strong> · Brand approval: <strong style={{ color: brandScriptApproved ? C.success : C.textMuted }}>{brandScriptApproved ? 'Approved' : 'Pending'}</strong>
                                                  </div>
                                                </>
                                              )}
                                              {(scriptStatus === 'approved' || mutualScriptApproved) && (
                                                <button
                                                  onClick={() => {
                                        ...
                                        if (creatorDealLifecycle === 'deliverables') {
                                          const oppDeliverables = opp.deliverables?.length ? opp.deliverables : [{ format: 'TikTok video', count: 1 }];
                                            ...
                                            <>
                                              ...
                                              <div style={{ background:C.bg, border:`1px solid ${C.border}`, borderRadius:'8px', padding:'10px', marginBottom:'10px' }}>
                                                ...
                                                {oppDeliverables.map((d, di) => {
                                                  ...
                                                  const isValidIgUrl = (u: string) => /tiktok\.com\/@[^/]+\/video\/\d+/.test(u) || /vm\.tiktok\.com\/[A-Za-z0-9]+/.test(u);
                                                  const postId = link.match(/tiktok\.com\/@[^/]+\/video\/(\d+)/)?.[1];
                                                  return (
                                                    <div key={di} style={{ borderRadius:'8px', marginBottom:'8px', border:`1px solid ${status === 'approved' ? 'rgba(0,212,106,0.25)' : status === 'uploaded' ? 'rgba(0,149,246,0.25)' : C.border}`, overflow:'hidden' }}>
                                                      <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'8px 10px', background: status === 'uploaded' ? 'rgba(0,149,246,0.04)' : status === 'approved' ? 'rgba(0,212,106,0.04)' : 'transparent' }}>
                                                        ...
                                                        <div style={{ flex:1 }}>
                                                          ...
                                                          <div style={{ fontSize:'10px', color: status === 'approved' ? C.success : status === 'uploaded' ? C.primary : C.textMuted }}>
                                                            {status === 'approved' ? 'Approved by brand' : status === 'uploaded' ? 'Submitted — awaiting review' : status === 'linking' ? 'Enter your TikTok link below' : 'Not yet submitted'}
                                                          </div>
                                                        </div>
                                                        {status === 'pending' && (
                                                      ...
                                                      {status === 'linking' && (
                                                        <div style={{ padding:'10px', borderTop:`1px solid ${C.border}`, background:C.bg }}>
                                                          <div style={{ fontSize:'10px', color:C.textMuted, marginBottom:'6px' }}>Paste the link to your published TikTok video</div>
                                                          <div style={{ display:'flex', gap:'6px' }}>
                                                            <input
                                                              type="text"
                                                              value={inputVal}
                                                          ...
                                                          {inputVal && !isValidIgUrl(inputVal) && (
                                                            <div style={{ fontSize:'10px', color:'#ef4444', marginTop:'4px' }}>Must be a TikTok link (tiktok.com/@.../video/... or vm.tiktok.com/...)</div>
                                                          )}
                                                        </div>
                                                      )}
                                                      {(status === 'uploaded' || status === 'approved') && link && (
                                                        <div style={{ padding:'10px', borderTop:`1px solid ${C.border}`, background:C.bg, display:'flex', gap:'10px', alignItems:'flex-start' }}>
                                                          ...
                                                          <div style={{ flex:1, minWidth:0 }}>
                                                            <div style={{ fontSize:'10px', fontWeight:700, color:C.textMuted, marginBottom:'2px', textTransform:'uppercase', letterSpacing:'0.4px' }}>TikTok video</div>
                                                            <a href={link} target="_blank" rel="noopener noreferrer" style={{ fontSize:'10px', color:C.primary, textDecoration:'none', display:'block', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{postId ? `tiktok.com/@.../video/${postId}` : link}</a>
                                          ...
                                          <>
                                            ...
                                            {status === 'content_due' && (
                                              <div style={{ background:'rgba(59,130,246,0.06)', border:`1px solid rgba(59,130,246,0.2)`, borderRadius:'8px', padding:'12px', marginBottom:'12px' }}>
                                                ...
                                                <div style={{ fontSize:'11px', color:C.textSecondary, marginBottom:'8px' }}>Submit a link to your TikTok video featuring the product</div>
                                                <input
                                                  type="text"
                                                  value={deliverableLinkInputs[0] || ''}
                                                  onChange={e => setDeliverableLinkInputs(prev => ({ ...prev, 0: e.target.value }))}
                                                  placeholder="https://www.tiktok.com/@handle/video/1234567890"
                                                  style={{ width:'100%', background:C.surfaceAlt, border:`1px solid ${isValidTiktokVideoUrl(deliverableLinkInputs[0] || '') ? C.success : C.border}`, borderRadius:'6px', color:C.text, padding:'7px 10px', fontSize:'11px', fontFamily:'inherit', outline:'none', marginBottom:'8px', boxSizing:'border-box' }}
                                                />
                                                <button
                                                  disabled={!isValidTiktokVideoUrl(deliverableLinkInputs[0] || '')}
                                                  onClick={() => {
                                                  ...
                                                  style={{ width:'100%', background: isValidTiktokVideoUrl(deliverableLinkInputs[0] || '') ? C.primary : C.border, border:'none', borderRadius:'6px', padding:'8px', color:'#fff', fontWeight:600, cursor: isValidTiktokVideoUrl(deliverableLinkInputs[0] || '') ? 'pointer' : 'not-allowed', opacity: isValidTiktokVideoUrl(deliverableLinkInputs[0] || '') ? 1 : 0.5, fontSize:'11px' }}
                                                >
                                                  Submit Content
                                                </button>
                                              </div>
                                      ...
                                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        ...
                                        <button
                                          onClick={() => {
                                            ...
                                            if (pocHandle) {
                                              window.open(`https://www.tiktok.com/@${pocHandle}`, '_blank');
                                            }
                                            setPurchaseToast(`Opening ${activeDeal.poc?.name || 'POC'} on TikTok`);
                                            setTimeout(() => setPurchaseToast(null), 2000);
                                          }}
                              ...
                              collabSentNames.map((name, i) => {
                                ...
                                return (
                                  <div key={i} style={{ background: C.card, borderRadius: '12px', padding: '14px', marginBottom: '10px', border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${(creator?.handle ?? name).replace('@','').replace(/\s/g,'')}`} alt={name} style={{ width: '40px', height: '40px', borderRadius: '50%', background: C.surfaceAlt, border: `2px solid ${badgeColor}`, flexShrink: 0 }} />
                                    <div style={{ flex: 1 }}>
                          ...
                          return (
                            <div key={i} style={{ background: C.card, borderRadius: '12px', padding: '14px', marginBottom: '10px', border: `1px solid ${open ? 'rgba(94,106,210,0.4)' : C.border}` }}>
                              <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '10px' }}>
                                <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${creator.handle.replace('@','')}`} alt={creator.name} style={{ width: '40px', height: '40px', borderRadius: '50%', background: C.surfaceAlt, border: `2px solid ${badgeColor}` }} />
                                <div style={{ flex: 1 }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        ...
                        <div style={{ background:C.surface, borderRadius:'16px', padding:'24px', maxWidth:'480px', width:'95vw', maxHeight:'90vh', overflowY:'auto', border:`1px solid ${C.border}`, position:'relative' }}>
                          ...
                          <div style={{ marginBottom:'16px' }}>
                            ...
                            <div style={{ position:'relative', marginBottom:'8px' }}>
                              ...
                              <input type="text" value={newCampaignPocHandle} onChange={e=>setNewCampaignPocHandle(e.target.value)} placeholder="tiktokhandle" style={{ width:'100%', background:C.bg, border:`1px solid ${C.border}`, borderRadius:'8px', color:C.text, padding:'8px 10px 8px 24px', fontSize:'12px', fontFamily:'inherit', outline:'none', boxSizing:'border-box' as const }} />
                            </div>
                      ...
                      <div style={{ position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.75)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999, padding:'16px' }}>
                        <div style={{ background:C.surface, borderRadius:'16px', padding:'24px', maxWidth:'500px', width:'100%', maxHeight:'90vh', overflowY:'auto', border:`1px solid ${C.border}`, position:'relative' }}>
                          ...
                          <div style={{ display:'flex', gap:'12px', alignItems:'flex-start', marginBottom:'16px' }}>
                            <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${selectedProfileCreator.handle.replace('@','')}`} alt={selectedProfileCreator.name} style={{ width:'60px', height:'60px', borderRadius:'50%', background:C.surfaceAlt }} />
                            <div style={{ flex:1 }}>
                              <div style={{ fontSize:'16px', fontWeight:700, color:C.text, marginBottom:'4px' }}>{selectedProfileCreator.name}</div>
                              <a href={`https://www.tiktok.com/@${selectedProfileCreator.handle.replace('@', '')}`} target="_blank" rel="noopener noreferrer" style={{ fontSize:'12px', color:C.primary, textDecoration:'none' }}>{selectedProfileCreator.handle}</a>
                      ...
                      return (
                        <div key={i} data-creator-idx={origIdx} style={{ background: C.card, borderRadius: '12px', padding: '16px', marginBottom: '12px', border: `1px solid ${isNegotiating ? 'rgba(230,81,0,0.4)' : creator.featured ? 'rgba(0,102,204,0.3)' : C.border}` }}>
                          <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
                            <a href={`https://www.tiktok.com/@${creator.handle.replace('@', '')}`} target="_blank" rel="noopener noreferrer" title="View TikTok profile" style={{ flexShrink:0 }}>
                              <img
                                src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${creator.handle.replace('@', '')}`}
                                alt={creator.name}
                              ...
                              {brandDealPhase === 'brief' && (
                                <>
                                  ...
                                  <div style={{ marginBottom: '10px' }}>
                                    ...
                                    <textarea
                                      placeholder="e.g. 2 × TikTok videos, 3 × video cuts, 1 × static post"
                                  ...
                                  {/* Scripting mode — set BEFORE negotiation path */}
                                  <div style={{ marginBottom: '12px' }}>
                                    <div style={{ fontSize: '11px', color: C.textMuted, marginBottom: '6px', fontWeight: 600 }}>Scripting mode <span style={{ color: C.textSecondary }}>*</span></div>
                                    <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
                                      {[
                                        { key: 'non_negotiable', title: '1) Non-negotiable script', desc: 'Fixed script in deal details. Creator can only accept/reject.' },
                                        { key: 'discussion', title: '2) Discussion', desc: 'Both discuss in chatroom and both must explicitly approve script.' },
                                        { key: 'creator_freedom', title: '3) Creator freedom', desc: 'Creator proposes script; brand approves/revises.' },
                                      ].map((m) => (
                                        <button
                                          key={m.key}
                                          onClick={() => { if (brandDealKey) updateDeal(brandDealKey, { scriptMode: m.key as any }); }}
                                          style={{ textAlign:'left', background: scriptMode === m.key ? `${C.primary}10` : C.bg, border:`1px solid ${scriptMode === m.key ? `${C.primary}55` : C.border}`, borderRadius:'8px', padding:'8px', cursor:'pointer' }}
                                        >
                                          <div style={{ fontSize:'11px', fontWeight:700, color:C.text }}>{m.title}</div>
                                          <div style={{ fontSize:'10px', color:C.textSecondary, marginTop:'2px', lineHeight:1.35 }}>{m.desc}</div>
                                        </button>
                                      ))}
                                    </div>
                                  </div>
                                  {scriptMode === 'non_negotiable' && (
                                    <div style={{ marginBottom: '12px' }}>
                                      <div style={{ fontSize: '11px', color: C.textMuted, marginBottom: '4px', fontWeight: 600 }}>Fixed script text <span style={{ color: C.textSecondary }}>*</span></div>
                                      <textarea
                                        value={brandScriptText}
                                        onChange={(e) => { if (brandDealKey) updateDeal(brandDealKey, { brandScriptText: e.target.value, scriptDraft: e.target.value, scriptVersion: 1 }); }}
                                        placeholder="Paste the exact script creator must follow..."
                                        rows={4}
                                        style={{ width:'100%', background:C.bg, border:`1px solid ${C.border}`, borderRadius:'8px', color:C.text, padding:'8px 10px', fontSize:'12px', fontFamily:'inherit', outline:'none', resize:'none', boxSizing:'border-box' }}
                                      />
                                    </div>
                                  )}
                                  ...
                                  <div style={{ display: 'flex', gap: '8px' }}>
                                    <button
                                      onClick={() => { if (brandBriefTitle.trim() && brandBriefDeliverables.trim() && brandBriefAbout.trim() && brandBriefCampaignDesc.trim() && (scriptMode !== 'non_negotiable' || brandScriptText.trim().length >= 20)) setBrandDealPhase('offer'); }}
                                      style={{ flex: 1, background: brandBriefTitle.trim() && brandBriefDeliverables.trim() && brandBriefAbout.trim() && brandBriefCampaignDesc.trim() && (scriptMode !== 'non_negotiable' || brandScriptText.trim().length >= 20) ? C.warning : C.border, border: 'none', padding: '9px', borderRadius: '8px', color: '#fff', fontWeight: 600, cursor: brandBriefTitle.trim() && brandBriefDeliverables.trim() && brandBriefAbout.trim() && brandBriefCampaignDesc.trim() && (scriptMode !== 'non_negotiable' || brandScriptText.trim().length >= 20) ? 'pointer' : 'not-allowed', fontSize: '13px' }}
                                    >
                                      Continue to Offer
                                    </button>
...
<div style={{ marginBottom: '12px' }}>
                                      ...
                                      onClick={() => {
                                        ...
                                        if (brandDealKey) {
                                          ...
                                          updateDeal(brandDealKey, {
                                            ...
                                            brandScriptText: scriptMode === 'non_negotiable' ? brandScriptText : '',
                                            scriptDraft: scriptMode === 'non_negotiable' ? brandScriptText : (brandDeal?.scriptDraft || ''),
                                            scriptVersion: scriptMode === 'non_negotiable' ? 1 : (brandDeal?.scriptVersion || 0),
                              ...
                              {brandDealPhase === 'counter' && (
                                <>
                                  ...
                                  <div style={{ background:C.bg, border:`1px solid ${C.border}`, borderRadius:'8px', padding:'8px', marginBottom:'10px' }}>
                                    <button onClick={() => setShowScriptEditorCreator(v => !v)} style={{ width:'100%', background:'none', border:`1px solid ${C.border}`, borderRadius:'6px', padding:'6px', fontSize:'10px', fontWeight:700, color:C.text, cursor:'pointer', marginBottom:'6px' }}>
                                ...
                                <>
                                  ...
                                  {brandApprovalPhase === 'accepted' && (() => {
                                    switch(dealType) {
                                      ...
                                      case 'c2c_paid':
                                        return (
                                          <div>
                                            <div style={{ fontSize:'11px', fontWeight:700, color:C.textMuted, textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'8px' }}>Scripting Mode (Locked)</div>
                                            <div style={{ background:C.bg, border:`1px solid ${C.border}`, borderRadius:'8px', padding:'10px', marginBottom:'10px' }}>
                                              <div style={{ fontSize:'12px', fontWeight:700, color:C.text, marginBottom:'4px' }}>
                                                {scriptMode === 'non_negotiable' ? '1) Non-negotiable script' : scriptMode === 'discussion' ? '2) Discussion' : '3) Creator freedom'}
                                              </div>
                                              <div style={{ fontSize:'11px', color:C.textSecondary, lineHeight:1.4 }}>
                                                {scriptMode === 'non_negotiable' ? 'No chatroom negotiation on script. Script is a fixed deal detail.' : scriptMode === 'discussion' ? 'Both parties negotiate in chatroom and must both click script approval.' : 'Creator drafts script; brand approves or requests revision.'}
                                              </div>
                                              {scriptMode === 'non_negotiable' && brandScriptText && (
                                                <div style={{ marginTop:'8px', padding:'8px', borderRadius:'6px', background:'rgba(239,68,68,0.06)', border:'1px solid rgba(239,68,68,0.22)', fontSize:'11px', color:C.textSecondary, whiteSpace:'pre-wrap' }}>
                                                  {brandScriptText}
                                                </div>
                                              )}
                                            </div>
                                      ...
                                      <div style={{ marginTop:'12px' }}>
                                        ...
                                        <button
                                          onClick={() => {
                                            ...
                                            const existingMsgs = brandDeal?.chatMessages || [];
                                            const mode = scriptMode || 'discussion';
                                            if (brandDealKey) {
                                              updateDeal(brandDealKey, {
                                                escrowFunded: true,
                                                brandApprovalPhase: 'funded',
                                                creatorDealLifecycle: mode === 'non_negotiable' ? 'deliverables' : 'scripting',
                                                scriptStatus: mode === 'non_negotiable' ? 'approved' : 'draft',
                                                scriptMode: mode,
                                                brandScriptText: mode === 'non_negotiable' ? brandScriptText : '',
                                                scriptDraft: mode === 'non_negotiable' ? brandScriptText : (brandDeal?.scriptDraft || ''),
                                                scriptVersion: mode === 'non_negotiable' ? 1 : (brandDeal?.scriptVersion || 0),
                                                creatorScriptApproved: mode === 'non_negotiable' ? true : false,
                                                brandScriptApproved: mode === 'non_negotiable' ? true : false,
                                            ...
                                            setTimeout(() => setPurchaseToast(null), 4000);
                                            if (mode === 'discussion' || mode === 'creator_freedom') {
                                              setTimeout(() => setBrandApprovalPhase('script_review'), 1200);
                                            } else {
                                              setTimeout(() => setBrandApprovalPhase('reviewing'), 1200);
                                            }
                                          }}
                                  ...
                                  {(dealType === 'paid' || dealType === 'c2c_paid') && brandApprovalPhase === 'funded' && (
                                    <div style={{ textAlign:'center', padding:'16px 0' }}>
                                      ...
                                      <div style={{ fontSize:'12px', color:C.textSecondary, marginTop:'4px' }}>
                                        {scriptMode === 'non_negotiable' ? 'Non-negotiable script locked. Creator can start deliverables.' : 'Creator has been notified to submit script draft'}
                                      </div>
                                    </div>
                                  )}
                                  {(dealType === 'paid' || dealType === 'c2c_paid') && brandApprovalPhase === 'script_review' && (
                                    <div style={{ marginTop:'12px' }}>
                                      ...
                                      <div style={{ fontSize:'11px', color:C.textSecondary, marginBottom:'8px' }}>
                                        Mode: <strong style={{ color:C.text }}>{scriptMode === 'discussion' ? 'Discussion' : 'Creator freedom'}</strong>
                                      </div>
                                      ...
                                      <div style={{ display:'flex', gap:'8px', marginBottom:'8px' }}>
                                        <button
                                          ...
                                          onClick={() => {
                                            if (!brandDealKey) return;
                                            const nextBrandApproved = true;
                                            const bothApproved = scriptMode === 'discussion' ? (creatorScriptApproved && nextBrandApproved) : true;
                                            updateDeal(brandDealKey, {
                                              scriptStatus: bothApproved ? 'approved' : 'submitted',
                                              scriptApprovedAt: bothApproved ? new Date().toISOString() : undefined,
                                              brandScriptApproved: nextBrandApproved,
                                              creatorDealLifecycle: bothApproved ? 'deliverables' : 'scripting',
                                              brandApprovalPhase: bothApproved ? 'reviewing' : 'script_review',
                                            });
                                            firebaseSendNotification('Creator', 'application', bothApproved ? 'Script approved. You can now upload deliverables.' : 'Brand clicked: Yes, I approve this script.');
                                            setPurchaseToast(bothApproved ? 'Script approved' : 'Brand approval recorded. Waiting for creator approval.');
                                            setTimeout(() => setPurchaseToast(null), 2500);
                                          }}
                                        ...
                                        >
                                          {scriptMode === 'discussion' ? 'Yes, I approve this script' : 'Approve Script'}
                                        </button>
                                        <button
                                          disabled={!brandDeal?.scriptDraft}
                                          onClick={() => {
                                        ...
                                        <div style={{ marginTop:'12px' }}>
                                          ...
                                          {contentLink ? (
                                            <div style={{ background:C.bg, borderRadius:'8px', padding:'12px', border:`1px solid ${C.border}`, marginBottom:'10px' }}>
                                              <div style={{ fontSize:'12px', fontWeight:600, color:C.text, marginBottom:'4px' }}>TikTok Video</div>
                                      ...
                                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        ...
                                        <button
                                          onClick={() => {
                                            ...
                                            setPurchaseToast(`Opening ${brandDeal.poc?.name || 'POC'} on TikTok`);
                                            setTimeout(() => setPurchaseToast(null), 2000);
                                          }}
                                          style={{ background: C.primary, border: 'none', borderRadius: '6px', padding: '5px 12px', color: '#fff', fontSize: '10px', fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}
                                        >
                              ...
                              {BRAND_MARKETPLACE_CREATORS.filter((_, idx) => idx !== 0).slice(0, 2).map((c, si) => {
                                ...
                                return (
                                  <div key={si} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '7px 0', borderTop: si > 0 ? `1px solid ${C.border}` : 'none' }}>
                                    <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${c.handle.replace('@','')}`} alt={c.name} style={{ width: '32px', height: '32px', borderRadius: '50%', background: C.surfaceAlt }} />
                                    <div style={{ flex: 1 }}>
                            ...
                            return (
                              <div key={key} style={{ background:C.card, borderRadius:'12px', padding:'14px', marginBottom:'10px', border:`1px solid ${C.border}` }}>
                                ...
                                <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'10px' }}>
                                  <img
                                    src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${creatorHandle}`}
                                    alt={creatorName}
                                    style={{ width:'36px', height:'36px', borderRadius:'50%', background:C.surfaceAlt, flexShrink:0 }}
                                  />
                                  <div style={{ flex:1, minWidth:0 }}>
                                    <a
                                      href={`https://www.tiktok.com/@${creatorHandle}`}
                                      target="_blank"
                                      rel="noopener noreferrer"
                    ...
                    <div style={{ marginTop:'16px' }}>
                      ...
                      {sharedApplications.filter(a => a.status !== 'invited').length === 0 ? (
                        ...
                        const igUrl = app.creatorInstagramUrl || `https://www.tiktok.com/@${app.creatorHandle.replace('@', '')}`;
                        return (
          ...
          {activeView === 'messages' && (
            <>
              {activeCommunity === null && activeDmId === null ? (
                  ...
                  <div style={{ padding: '0' }}>
                    ...
                    {messagesTab === 'communities' && (
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        {hasVisibleValueSkin && messagesTab === 'communities' && (
                        ...
                        {(() => {
                          const userProfessions = visibleOwnedSkins.map(s => s.profession);
                          const matchedChannels = CHANNELS.filter(ch =>
                            ch.gateType === 'any_valueskin' ? hasVisibleValueSkin :
                            ch.allowedProfessions.some(p => userProfessions.includes(p))
                          );
                          if (!hasVisibleValueSkin) return (
                            <div style={{ textAlign: 'center', padding: '40px 20px', color: C.textMuted }}>
                ...
                <>
                  {/* DM Chat View — exact TikTok DM UI */}
                  {(() => {
          ...
          {activeView === 'admin' && (
            <>
              ...
              <div style={{ padding: '20px', borderTop: `1px solid ${C.border}` }}>
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <div style={{ fontSize: '14px', fontWeight: 700, color: C.text }}>Platform DMs (TikTok, etc.)</div>
                ...
                {exploreTab === 'trending' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {[
                      ...
                      { title: 'Design Portfolios on TikTok', desc: 'UX designers showcase work through video-first formats', tag: 'Art & Design', views: '9K' },
                    ...
                    {BRAND_MARKETPLACE_CREATORS.map((c, i) => {
                      return (
                        <div key={i} onClick={() => setPreviewCreator(c)}
                          ...
                          onMouseEnter={e => { e.currentTarget.style.borderColor = C.primary; }}
                          onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; }}>
                          <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${c.handle.replace('@', '')}`} alt={c.name} style={{ width: '44px', height: '44px', borderRadius: '50%', background: C.surfaceAlt }} />
                          <div style={{ flex: 1 }}>
            ...
            <Modal onClose={() => setPreviewCreator(null)}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
                  <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${previewCreator.handle.replace('@', '')}`} alt={previewCreator.name}
                ...
                {/* Actions */}
                <div style={{ display: 'flex', gap: '8px' }}>
                  <a href={`https://www.tiktok.com/@${previewCreator.handle.replace('@', '')}`} target="_blank" rel="noopener noreferrer" style={{ flex: 1, padding: '12px', borderRadius: '8px', border: `1px solid ${C.border}`, background: C.surfaceAlt, color: C.primary, fontSize: '14px', fontWeight: 600, cursor: 'pointer', textAlign: 'center', textDecoration: 'none' }}>Visit TikTok</a>
      ...
      {showSkinManageModal && (
        <SkinManagementModal
          ...
          onDelete={(slot) => {
            const deletedProfession = valueSkins[slot]?.profession;
            setValueSkins(prev => {
              const next = { ...prev };
              delete next[slot];
              return next;
            ...
            if (deletedProfession) {
              setBrandValueSkins(prev => {
                const next = prev.filter(s => s !== deletedProfession);
                setActiveBrandSkin(current => {
                  if (current !== deletedProfession) return current;
                  return next[0] ?? null;
                });
                return next;
              });
              setSelectedMarketplaceSkin(prev => (prev === deletedProfession ? null : prev));
            }
            setHiddenSkins(prev => {
              const next = new Set(prev);
              next.delete(slot);
...
function NavItem({ label, active, onClick, badgeCount }: { label: string; active: boolean; onClick: () => void; badgeCount?: number }) {
  ...
  const icon = (() => {
    ...
    if (label === 'Profile') return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconStroke} strokeWidth="2"><circle cx="12" cy="8" r="4"/><path d="M4 21a8 8 0 0 1 16 0"/></svg>;
    if (label === 'Marketplace') return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconStroke} strokeWidth="2"><path d="M6 7h12l-1 13H7L6 7z"/><path d="M9 7a3 3 0 0 1 6 0"/></svg>;
    if (label === 'Store') return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconStroke} strokeWidth="2"><path d="M3 9h18l-2 12H5L3 9z"/><path d="M8 9V5h8v4"/></svg>;
    if (label === 'Settings') return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconStroke} strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .34 1.87l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.7 1.7 0 0 0-1.87-.34 1.7 1.7 0 0 0-1 1.55V22a2 2 0 1 1-4 0v-.09a1.7 1.7 0 0 0-1-1.55 1.7 1.7 0 0 0-1.87.34l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.7 1.7 0 0 0 .34-1.87 1.7 1.7 0 0 0-1.55-1H2a2 2 0 1 1 0-4h.09a1.7 1.7 0 0 0 1.55-1 1.7 1.7 0 0 0-.34-1.87l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.7 1.7 0 0 0 1.87.34h.01a1.7 1.7 0 0 0 1-1.55V2a2 2 0 1 1 4 0v.09a1.7 1.7 0 0 0 1 1.55h.01a1.7 1.7 0 0 0 1.87-.34l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.7 1.7 0 0 0-.34 1.87v.01a1.7 1.7 0 0 0 1.55 1H22a2 2 0 1 1 0 4h-.09a1.7 1.7 0 0 0-1.55 1z"/></svg>;
    if (label === 'Admin Panel') return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconStroke} strokeWidth="2"><path d="M12 3l8 4v5c0 5-3.5 8-8 9-4.5-1-8-4-8-9V7l8-4z"/></svg>;
  ...
  return (
    <button
      onClick={onClick}
      style={{ background: active ? `${C.primary}10` : 'transparent', border: active ? `1px solid ${C.primary}30` : '1px solid transparent', borderRadius: '12px', padding: '11px 12px', color: active ? C.text : C.textSecondary, fontWeight: active ? 700 : 500, fontSize: '15px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', transition: 'all 0.15s', width: '100%', justifyContent: 'flex-start', position: 'relative' }}
      onMouseEnter={(e) => { if (!active) { e.currentTarget.style.background = C.surfaceAlt; e.currentTarget.style.color = C.text; } }}
      onMouseLeave={(e) => { if (!active) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = C.textSecondary; } }}
    >
      <span style={{ fontSize: '16px', lineHeight: 1, display: 'inline-flex' }}>{icon}</span>
      <span>{label}</span>
      {badgeCount !== undefined && badgeCount > 0 && (
        <span style={{ minWidth: '18px', height: '18px', borderRadius: '9px', background: C.primary, color: '#fff', fontSize: '10px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 5px', marginLeft: 'auto' }}>{badgeCount}</span>
      )}
    </button>
  );
}
    ...
    <Modal onClose={onClose}>
      <div style={{ paddingBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          {/* Info */}
          <div style={{ fontSize: '11px', color: C.textMuted, padding: '10px', background: C.surfaceAlt, borderRadius: '6px', lineHeight: 1.5 }}>
...
function Modal({ onClose, children }: { onClose: () => void; children: React.ReactNode }) {
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(22,24,35,0.48)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: C.card, borderRadius: '16px', padding: '22px', maxWidth: '500px', width: '95vw', maxHeight: '90vh', overflowY: 'auto', position: 'relative', border: `1px solid ${C.border}`, boxShadow: '0 20px 40px rgba(0,0,0,0.18)' }}>
